"""
RGB-D Salient Object Detection - NLPR Test Set Inference
Processes all 300 NLPR test images from mentor's dataset
"""
...
def main():
    print("="*70)
    print("RGB-D SOD - NLPR TEST SET INFERENCE (300 IMAGES)")
    print("="*70)
    
    # Configuration
    TEST_DATA_DIR = "./nlpr_test_data"
    RGB_DIR = os.path.join(TEST_DATA_DIR, "RGB")
    DEPTH_DIR = os.path.join(TEST_DATA_DIR, "depth")
    GT_DIR = os.path.join(TEST_DATA_DIR, "GT")
    ...
    print(f"  ✓ Loaded {len(test_triplets)} complete test triplets (RGB + Depth + GT)")
    ...
    for idx, (rgb_path, depth_path, gt_path) in enumerate(tqdm(test_triplets, desc="Processing")):
        try:
            ...
            # Save output
            base_name = os.path.splitext(os.path.basename(rgb_path))[0]
            output_path = os.path.join(OUTPUT_DIR, "predictions", f"{base_name}_pred.png")
    ...
    print(f"  • Total time: {elapsed:.1f}s ({elapsed/len(test_triplets):.2f}s per image)")
    ...
    stats = {
        'dataset': 'NLPR Test Set (Official 300 images)',
    ...
    # Print summary
    print("\n" + "="*70)
    print("INFERENCE RESULTS - NLPR TEST SET (300 IMAGES)")
    print("="*70)
    print(f"\nDataset: NLPR Official Test Set")
    ...
    print(f"  • predictions/ - 300 prediction images (*_pred.png)")
    print(f"  • test_results.json - detailed metrics for all images")
    print("="*70)
